...
pub struct ImguiRenderer {
    ...
    pub platform: winit_support::WinitPlatform,
    renderer: imgui_wgpu::Renderer,
}

impl ImguiRenderer {
    pub fn new(graphics: &crate::Instance) -> Self {
        let mut ctx = imgui::Context::create();
        ctx.set_ini_filename(None);
        let mut platform = winit_support::WinitPlatform::init(&mut ctx);
        platform.attach_window(
            ctx.io_mut(),
            &graphics.window,
            winit_support::HiDpiMode::Default,
        );
    ...
    pub fn update(&mut self, window: &nano::Window, delta: std::time::Duration) {
        self.platform
    ...
    pub fn new_ui_frame(&mut self) -> &mut Ui {
        self.imgui.new_frame()
    }

    pub fn draw<'a>(
        &'a mut self,
        graphics: &crate::Instance,
        ...
        self.renderer
            ...
            .map_err(crate::Error::Imgui)?;
        Ok(())
    }

    pub fn handle_event(&mut self, window: &nano::Window, event: &nano::Event) {
        self.platform
            .handle_event(self.imgui.io_mut(), window, event);
    }
}

// Stolen from imgui_winit_support crate because i couldn't get it compiling on my machine
mod winit_support {
    use imgui::{self, BackendFlags, ConfigFlags, Context, Io, Key, Ui};
    use nano::winit::dpi::{LogicalPosition, LogicalSize};
